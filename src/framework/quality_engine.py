"""
Converts YAML quality expectations into SDP Expectations dicts.

Spark Declarative Pipelines (pyspark.pipelines) uses these decorators:
  @dp.expect_all({"name": "constraint", ...})           -> warn (log but keep)
  @dp.expect_all_or_drop({"name": "constraint", ...})   -> drop bad rows
  @dp.expect_all_or_fail({"name": "constraint", ...})   -> fail pipeline
"""
...
def apply_expectations(table_config: TableConfig) -> dict:
    """
    Group expectations by action type for use with SDP decorators.
