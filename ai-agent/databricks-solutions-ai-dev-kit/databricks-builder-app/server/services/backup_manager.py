...
def _get_projects_base_dir() -> Path:
  """Resolve the base directory for generated project folders."""
  return Path(os.getenv('PROJECTS_BASE_DIR', './projects')).resolve()


def _get_workspace_override_dir() -> Path | None:
  """Resolve optional shared workspace root override directory."""
  override = os.getenv('PROJECT_WORKSPACE_ROOT', '').strip()
  if not override:
    return None
...
def mark_for_backup(project_id: str) -> None:
  ...
  """
  if _get_workspace_override_dir():
    # In workspace-override mode, skip DB backups for the shared local repo path.
    return
...
async def create_backup(project_id: str) -> bool:
  ...
  """
  if _get_workspace_override_dir():
    logger.debug('Workspace override mode enabled, skipping create_backup')
    return False

  project_dir = _get_projects_base_dir() / project_id
...
async def restore_backup(project_id: str) -> bool:
  ...
  """
  if _get_workspace_override_dir():
    logger.debug('Workspace override mode enabled, skipping restore_backup')
    return False
...
def ensure_project_directory(project_id: str) -> Path:
  ...
  workspace_override = _get_workspace_override_dir()
  if workspace_override:
    workspace_dir = workspace_override
    workspace_dir.mkdir(parents=True, exist_ok=True)
    if not (workspace_dir / '.claude' / 'skills').exists():
      copy_skills_to_project(workspace_dir)
    if not (workspace_dir / 'CLAUDE.md').exists():
      _create_default_claude_md(workspace_dir)
