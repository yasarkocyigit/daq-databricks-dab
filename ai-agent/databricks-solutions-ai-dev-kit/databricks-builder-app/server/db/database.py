...
def is_sqlite_mode() -> bool:
    """Check whether the configured database URL uses SQLite."""
...
def is_postgres_configured() -> bool:
    """Check if a database is configured (static URL or dynamic OAuth)."""
    return bool(
        get_database_url()
        or (
...
def run_migrations() -> None:
    ...
    if is_sqlite_mode():
        logger.info("SQLite mode detected. Skipping Alembic migrations.")
        return
